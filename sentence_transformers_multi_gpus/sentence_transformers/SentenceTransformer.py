...
#             modules_config.append({'idx': idx, 'name': name, 'path': os.path.basename(model_path), 'type': "sentence_transformers.models.Transformer"})
        
        # edited
        new_modules_config = []
        for i in modules_config:
            fix = i.copy()
            fix['type'] = fix['type'].replace('sentence_transformers_multi_gpus.', '')
            new_modules_config.append(fix)
            
        with open(os.path.join(path, 'modules.json'), 'w') as fOut:
            json.dump(new_modules_config, fOut, indent=2)
            
        with open(os.path.join(path,'1_Pooling', 'config.json'), 'r') as fp:
            current_pooling_config= json.load(fp)
            
        with open(os.path.join(path,'1_Pooling', 'config.json'), 'w') as fOut:
            json.dump({'word_embedding_dimension': current_pooling_config['word_embedding_dimension'],
                       'pooling_mode_cls_token': current_pooling_config['pooling_mode_cls_token'],
                       'pooling_mode_mean_tokens': current_pooling_config['pooling_mode_mean_tokens'],
                       'pooling_mode_max_tokens': current_pooling_config['pooling_mode_max_tokens'],
                       'pooling_mode_mean_sqrt_len_tokens': current_pooling_config['pooling_mode_mean_sqrt_len_tokens'],
                      }, fOut, indent=2)
